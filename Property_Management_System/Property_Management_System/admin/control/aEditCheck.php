<?php
    ...
    $userQuery1=$connection->searchAgent($conobj,"agent",$a_id);
    ...
    if ($userQuery1->num_rows!=null) {
        ...
        while($row = $userQuery1->fetch_assoc()) {
            ...
            $fname=$row["a_firstname"];
            $lname = $row["a_lastname"];
            $email=$row["a_email"];
            $dob =$row["a_dob"];
            $phone=$row["a_phone"];
            $aphoto= $row["a_photo"];
    ...
    if (isset($_POST['update'])) {
        ...
        if(!$hasError){
            $connection = new db();
            $conobj=$connection->OpenCon();
            $userQuery=$connection->UpdateUser($conobj,"agent",$_POST["username"],$_POST['fname'],$_POST['lname'], $_POST['dob'],$_POST['gender'],$_POST['phone'], $_POST['email']);
            if($userQuery==TRUE)
            {
